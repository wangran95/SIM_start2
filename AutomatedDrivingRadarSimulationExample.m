...
Model the radar transmitter for a single transmit channel, and model a receiver preamplifier for each receive channel, using the parameters defined in the example docid:phased_examples.example-ex11665805.
...
The radar collects multiple sweeps of the waveform on each of the linear phased array antenna elements. These collected sweeps form a data cube, which is defined in docid:phased_gs.bsmig5l-1. These sweeps are coherently processed along the fast- and slow-time dimensions of the data cube to estimate the range and Doppler of the vehicles.
...
where  is the RMS range resolution and  is the Rayleigh range resolution.
...
Create a highway driving scenario with three vehicles traveling in the vicinity of the ego vehicle. The vehicles are modeled as point targets, and have different velocities and positions defined in the driving scenario. The ego vehicle is moving with a velocity of 80 km/hr and the other three cars are moving at 110 km/hr, 100 km/hr, and 130 km/hr respectively. For details on modeling a driving scenario see the example Create Actor and Vehicle Paths. The radar sensor is mounted on the front of the ego vehicle.
